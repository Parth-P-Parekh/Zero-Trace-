...
**7. Marketing is Persuade, and gets exactly one bold move.** The hero is the drain itself: a single line of live payload text where the sensitive runs mask in place on load, left to right, on the `--d-drain` 900ms sweep, once. No big-number hero, no feature-icon grid, no gradient. The headline uses the reference chrome's treatment the readme names — first clause at full ink, continuation at ramp `.36`, fading like the mark.
