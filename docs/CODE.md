...
zerotrace/
  ...
  requirements.in                direct dependencies, input to pip-compile              [A+]
  requirements.txt               pinned, generated by pip-compile                       [A]
  .env.example                   every variable, with a safe default or an explicit TODO [A]
  alembic.ini                    [A]
  ...
  zerotrace/
    config.py                    Settings(pydantic-settings); one source of truth for env   [A]
    clock.py                     now() — injectable, UTC                                    [A]
    errors.py                    ZTError hierarchy; every error carries a degrade_reason     [A]
    logging.py                   structlog JSON + a redacting processor (§21.3)             [A]
    ids.py                       ULID for req_<ulid> / ses_<ulid>                            [A+]

    gateway/                     C1, C2, C9 — the interception layer
      app.py                     FastAPI app, lifespan, middleware order                    [A]
      deps.py                    FastAPI dependency providers: session, detector, upstream  [A+]
      redact.py                  Part A's subset of S5: mask/block. Superseded by denormalise.py at M3  [A+]
      upstream.py                stub + httpx passthrough; the real client lands at M5       [A+]
      routes_dataplane.py        /v1/chat/completions, /v1/messages, /v1/embeddings, /v1/responses
      routes_control.py          mounts the control-plane routers (§15.2)                   [A]
    ...
    spans/
      model.py                   Span, SpanTree, Finding, Decision                          [A: Finding, Decision]
      paths.py                   span_path grammar, parse/format, safe indexing             [A]
      pathsafe.py                generalise non-vocabulary path segments (§5.2)

    detect/
      stub.py                    the detection seam + its declared no-op (Part A)          [A+]
    ...
    policy/
      schema.py                  pydantic models for the YAML in PROD-01 §9                 [A]
      engine.py                  C7 — resolution, inheritance, action lattice               [A]
      store.py                   versioned load/save, cache invalidation                    [A]
      exceptions.py              scoped exceptions, approval routing                        [A]
    ...
    identity/                    C22
      oidc.py                    OIDC login, session cookies                                [A: seeded dev token only]
      scim.py                    SCIM 2.0 Users + Groups
      workload.py                SPIFFE/mTLS identity for service accounts                  [A: wired, inert in dev]
      resolve.py                 request → Actor (the one function the hot path calls)      [A]
    ...
    ledger/
      chain.py                   C13 — append, hash chain, verify                           [A]
      records.py                 record schemas per event_type                              [A]
...
-- Added by SKEL-01 Part A (migration 001). A group is data, not an enum value in
-- code, so an administrator adds one without a build. actors.groups holds the
-- membership; this table exists so the console can list the groups a tenant has
-- defined without scanning every actor row.
